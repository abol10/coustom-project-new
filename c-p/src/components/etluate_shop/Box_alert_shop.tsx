...
const Box_Alert_shop: React.FC<ChildProps> = ({ setChekednav, state }) => {
    const navigate = useNavigate();

    const handleAlert = () => {
        // نمایش SweetAlert2 برای موفقیت با اندازه کوچکتر
        Swal.fire({
            ...
            setChekednav(false);
            if (state) {
                navigate('/'); // هدایت به صفحه /E در صورت true بودن state
            }
        });
    };
